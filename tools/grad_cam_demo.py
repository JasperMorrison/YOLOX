...
import os
import time
from tools.grad_cam import Grad_CAM
...
def make_parser():
    ...
    parser.add_argument(
        ...
        nargs='+',
        type=int,
        help='grad cam, filter by class: --class 0, or --class 0 2 3'
    )
    return parser
...
class Predictor(object):
    ...
    def inference(self, img):
        ...
        img_info['input_size'] = self.test_size
    ...
    def backward(self, output, img_info, class_id=None, cls_conf=0.35):
        ratio = img_info['ratio']
        img = img_info['raw_img']
        if output is None:
            logger.warning("output is None")
        ...
        for cls_index, score in enumerate(scores):
            cls_id = cls[cls_index]
            if score * output[:,4][cls_index] < cls_conf:
                continue
            if class_id and cls_id not in class_id:
                continue
            c_id = int(cls_id.cpu().item())
            logger.info("backward with class " + str(c_id) + ":" + COCO_CLASSES[c_id])
            score.backward(retain_graph=True)
            # cls_id.backward(retain_graph=True)
            # for box in bboxes:
            #     for location in box:
            #         location.backward(retain_graph=True)

def image_demo(predictor, vis_folder, path, current_time, save_result, classes):
    ...
    for image_name in files:
        grad_cam = Grad_CAM()
        grad_cam.grad_cam_register(predictor.model, cam_layer)
        outputs, img_info = predictor.inference(image_name)
        result_image = predictor.visual(outputs[0], img_info)
        predictor.backward(outputs[0], img_info, classes)
        grad_cam.save_cam("./cam_output", img_info)
        grad_cam.grad_cam_remove()
        if save_result:
...
def main(exp, args):
    ...
    if args.demo == 'image':
        image_demo(predictor, vis_folder, args.path, current_time, args.save_result, args.classes)
